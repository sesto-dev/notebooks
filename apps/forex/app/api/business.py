...
from constants import TIMEZONE, RETCODES
from library.telegram import TelegramSender
from api.data import symbol_info_tick
from api.fetch import _make_request

def order_send(request):
    endpoint = f'/order_send'
    return _make_request('POST', endpoint, params=request)

def last_error():
    endpoint = f'/last_error'
    return _make_request('GET', endpoint)

def last_error_str():
    endpoint = f'/last_error_str'
    return _make_request('GET', endpoint)

def positions_total():
    endpoint = f'/positions_total'
    return _make_request('GET', endpoint)

def positions_get():
    endpoint = f'/positions_get'
    return _make_request('GET', endpoint)

def history_deals_get():
    endpoint = f'/history_deals_get'
    return _make_request('GET', endpoint)

def history_orders_get():
    endpoint = f'/history_orders_get'
    return _make_request('GET', endpoint)

def send_market_order(symbol, volume, order_type, sl=0.0, tp=0.0,
                      deviation=20, comment='', magic=0, type_filling='ORDER_FILLING_IOC'):
    ...
    order_type_dict = {'buy': 'ORDER_TYPE_BUY', 'sell': 'ORDER_TYPE_SELL'}
    ...
    if order_result is None:
        ...
        error_description = last_error_str()
        print(f"Order failed, error code: {error_code}, description: {error_description}")
        return None
    elif order_result.retcode != 'TRADE_RETCODE_DONE':
        Telegram = TelegramSender()
        Telegram.send_json_message({"event": "error with sending order", "retcode": order_result.retcode, "description": RETCODES.get(order_result.retcode, 'Unknown'), "request": request})
        return None
...
def close_position(position, deviation=20, magic=0, comment='', type_filling='ORDER_FILLING_IOC'):
    if 'type' not in position or 'ticket' not in position:
        print("Position dictionary missing 'type' or 'ticket' keys.")
        return None

    order_type_dict = {
        0: 'ORDER_TYPE_BUY',   # Corrected
        1: 'ORDER_TYPE_SELL'   # Corrected
    }
    ...
    tick = symbol_info_tick(position['symbol'])
    if tick is None:
...
def close_all_positions(order_type='all', magic=None, type_filling='ORDER_FILLING_IOC'):
    order_type_dict = {
        'buy': 'ORDER_TYPE_BUY',
...
def get_order_from_ticket(ticket):
    ...
    order = history_orders_get(ticket=ticket)
